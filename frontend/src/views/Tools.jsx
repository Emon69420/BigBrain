...
export function ToolsView(){
  ...
  async function load(){ setStats(await fetch(`${import.meta.env.VITE_API_URL||"http://localhost:8000"}/tools/stats`,{headers:{"Content-Type":"application/json","X-Org-Id":api.getOrg()},credentials:"include"}).then(r=>r.json())); }
  useEffect(()=>{ load(); },[]);
  async function ensure(e){
    ...
    setTrace(["ensuring…"]);
    const r=await fetch(`${import.meta.env.VITE_API_URL||"http://localhost:8000"}/tools/ensure`,{method:"POST",headers:{"Content-Type":"application/json","X-Org-Id":api.getOrg()},credentials:"include",body:JSON.stringify({task, sample_input:sample})}).then(r=>r.json());
    setLast(r);
    setTrace(r.trace||[]);
    load();
  }
  async function run(name, argsRaw){
    ...
    const r=await fetch(`${import.meta.env.VITE_API_URL||"http://localhost:8000"}/tools/run`,{method:"POST",headers:{"Content-Type":"application/json","X-Org-Id":api.getOrg()},credentials:"include",body:JSON.stringify({name,args})}).then(r=>r.json());
    setRunRes(r); load();
  }
