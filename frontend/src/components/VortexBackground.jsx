// Ported from 21st.dev community component:
//   "Interactive Neural Vortex Background" by Le Thanh (minhxthanh)
//   https://21st.dev/@minhxthanh/components/interactive-neural-vortex-background
// License as listed on 21st.dev: unknown — attribution kept here. Review before commercial use.
// What changed from the original: removed their hero section, Tailwind classes,
// styled-jsx block and Geist fonts; kept the WebGL canvas setup, shaders,
// pointer interaction and cleanup verbatim. Wrapped in a plain absolutely-positioned
// container driven by our own CSS tokens (opacity prop, default low per design spec §5a).
import React, { useEffect, useRef } from 'react';

export default function VortexBackground({ opacity = 0.3, className = "" }) {
  ...
  useEffect(() => {
    ...
    if (reduceMotion) {
      // static single frame per design spec §11
      gl.uniform1f(uTime, 4000);
      gl.uniform2f(uPointerPosition, 0.5, 0.5);
  ...
  return (
    <div className={`vortex-host ${className}`} aria-hidden="true">
      <canvas ref={canvasRef} className="vortex-canvas" style={{ opacity }} />
    </div>
  );
}
